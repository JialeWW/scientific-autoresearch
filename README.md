...
`scientific-autoresearch` is a general-purpose skill for autonomous scientific research agents. It helps an agent move from a scientific question to mechanism-first hypotheses, testable claims, falsification checks, conservative interpretation, and reproducible round-by-round reports.

The skill is discipline-agnostic and is designed to make an agent behave more like a careful research collaborator: curious, skeptical, reproducible, and slow to overclaim.

The project was inspired by Andrej Karpathy's
[`autoresearch`](https://github.com/karpathy/autoresearch) project and adapts
agent-run iterative research to discipline-agnostic scientific work.
...
This repository provides a promptable research protocol for LLM-based research assistants that can read Markdown instructions and work with local files. It can be used across model providers and agent platforms.

## Privacy and Safety Notes

The release package contains Markdown instructions, documentation, citation
metadata, a license, and workflow diagram assets. It contains no private
datasets, API keys, telemetry code, or model-provider-specific configuration.
When applying the skill to real projects, do not publish confidential inputs,
human-subject identifiers, credentials, unreleased manuscript notes,
proprietary data, or absolute local file paths in shared reports. Prefer
relative paths, redacted identifiers, and hashes when reproducibility artifacts
may become public.
...
The workflow diagram is included below and as a standalone PDF:
[`docs/scientific_autoresearch_flow.pdf`](docs/scientific_autoresearch_flow.pdf).
...
|-- .gitignore
|-- docs/
|   |-- scientific_autoresearch_flow.pdf
...
agent-run iterative research to discipline-agnostic scientific work: it shifts
the emphasis from single-metric code optimization to mechanism-first claims,
support audits, falsification checks, uncertainty tracking, and conservative
interpretation. This repository is an independent project and is not a fork of,
affiliated with, or endorsed by the Karpathy `autoresearch` project.

## Author and Citation

Author: Jiale Wang ([ORCID: 0009-0007-0367-0631](https://orcid.org/0009-0007-0367-0631))
...
GitHub release; when an archival DOI is available, cite the DOI for the exact
version used.
