#!/usr/bin/env python3
"""Freeze the protocol-2.1.1 non-development shakedown manifest and artifacts."""
...
SOURCE_MANIFEST_PATH = HERE / "manifest-v2.1.1.json"
HARNESS_PATH = HERE / "shakedown-harness.py"
SPEC = importlib.util.spec_from_file_location(
    "scientific_autoresearch_benchmark_score_2_1_1_prepare", SCORER_PATH
)
...
def prepare(output_manifest: Path, frozen_at: str) -> dict[str, Any]:
    ...
    artifact_root = HERE / "artifacts" / "shakedown-v2.1.1"
    ...
    judge_prompt_path = HERE / "fixtures" / "shakedown-v2.1.1" / "judge-prompt.txt"
    ...
    trigger_cases = HERE / "fixtures" / "shakedown-v2.1.1" / "trigger-canary.json"
    behavioral_cases = (
        HERE / "fixtures" / "shakedown-v2.1.1" / "behavioral-canary.json"
    )
    trigger_suite = freeze_suite(
        source_suites["trigger-routing"],
        suite_id="shakedown-trigger-routing",
        case_path=trigger_cases,
        collection_key=None,
        prompt_field="query",
        repetitions=1,
        block_id="shakedown-trigger-block-v2.1.1",
        order_seed=2111,
    ...
    manifest = {
        ...
        "protocol_id": "scientific-autoresearch-benchmark-v2.1.1",
        "release_under_test": "0.2.8",
        "benchmark_status": "execution_shakedown_frozen",
        "manifest_kind": "execution",
        "execution_manifest_id": "scientific-autoresearch-shakedown-v2.1.1-20260722",
