#!/usr/bin/env python3
"""Score hash-bound, paired benchmark attempts against protocol 2.1.1."""
...
def _evidence_tree_snapshot(root: Path) -> tuple[str, dict[str, str]]:
    ...
    for current, directory_names, file_names in os.walk(
        ...
        for directory_name in directory_names:
            ...
            if directory_path.is_symlink() or not directory_path.is_dir():
                raise ValueError("evidence tree cannot contain linked or special directories")
        for file_name in file_names:
            file_path = current_path / file_name
            if file_path.is_symlink():
                raise ValueError("evidence tree cannot contain symbolic links")
    ...
    for index, record in enumerate(records, start=1):
        ...
        for kind, path_field, hash_field in references:
            ...
            if previous is not None:
                errors.append(f"record {index} {kind} path is referenced more than once")
...
def _load_protocol(
    ...
    manifest_payload = _stable_file_bytes(
        manifest_path,
        "benchmark manifest",
        maximum_bytes=16 * 1024 * 1024,
    )
    ...
    if raw.get("benchmark_protocol_version") != "2.1.1":
        raise ValueError("manifest must bind benchmark_protocol_version=2.1.1")
    ...
    for index, suite_raw in enumerate(suites_raw, start=1):
        ...
        if backend == "score.py":
            ...
            if execution_purpose == "execution_shakedown":
            ...
            role = str(suite.get("case_source", {}).get("role", ""))
            if execution_purpose == "execution_shakedown" and role != "shakedown":
                raise ValueError(f"execution shakedown suite {suite_id} must use shakedown cases")
            if execution_purpose in {"development_evaluation", "sealed_evaluation"}:
                expected_role = (
                    "development" if execution_purpose == "development_evaluation" else "sealed"
                )
                if role != expected_role:
                    raise ValueError(
                        f"suite {suite_id} case role does not match execution purpose"
                    )
            if manifest_kind == "protocol_template" and role == "shakedown":
                raise ValueError("protocol templates cannot use shakedown cases")
    ...
    if manifest_kind == "protocol_template":
        if global_block_ids or any(
            suite.get("scoring_backend") == "score.py"
            and suite.get("protocol_status") != "awaiting_execution_freeze"
            for suite in suites.values()
        ):
            raise ValueError("protocol template cannot contain frozen execution blocks")
    elif not global_block_ids:
        raise ValueError("execution manifest needs at least one frozen comparison block")
...
def score_records(
    ...
    *,
    ...
    expected_evidence_files, evidence_reference_errors = _expected_evidence_files(
        records
    )
    fatal_errors.extend(evidence_reference_errors)
    ...
    try:
        evidence_tree_sha256, actual_evidence_files = _evidence_tree_snapshot(
            evidence_root
        )
        missing_files = sorted(set(expected_evidence_files) - set(actual_evidence_files))
        orphan_files = sorted(set(actual_evidence_files) - set(expected_evidence_files))
        mismatched_files = sorted(
            relative
            for relative in set(expected_evidence_files) & set(actual_evidence_files)
            if expected_evidence_files[relative] != actual_evidence_files[relative]
        )
        if missing_files:
            fatal_errors.append(
                f"evidence tree is missing referenced files: {missing_files[:10]}"
            )
        if orphan_files:
            fatal_errors.append(
        ...
        if mismatched_files:
            fatal_errors.append(
                f"evidence tree content differs from record bindings: {mismatched_files[:10]}"
            )
    except ValueError as exc:
    ...
    bound_paths: dict[str, tuple[str, str, int, tuple[str, str]]] = {}
    ...
    for index, row in enumerate(normalized, start=1):
        ...
        for path_kind, bound_path in (
            ("evidence", row["__evidence_absolute__"]),
            ("judgment", row.get("__judgment_absolute__")),
        ):
            if not bound_path:
                continue
            if bound_path in bound_paths:
                _, previous_kind, previous_index, previous_key = bound_paths[bound_path]
                invalidate_blocks(
                    block_key,
                    previous_key,
                    f"records {previous_index} and {index}: one file was reused as {previous_kind} and {path_kind}",
                )
            bound_paths[bound_path] = (identity, path_kind, index, block_key)
...
def _self_test() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="scientific-autoresearch-score-") as temporary:
        ...
        harness_path = artifacts / "self-test-harness.py"
        harness_path.write_text("HARNESS_VERSION = '1'\n", encoding="utf-8")
        package_path = artifacts / "self-test-package.zip"
        with zipfile.ZipFile(package_path, "w", compression=zipfile.ZIP_STORED) as archive:
            for name, payload in sorted(
                {
                    "SKILL.md": b"---\nname: self-test\n---\n",
                    "references/reference.md": b"reference\n",
                    "scripts/check.py": b"VALUE = 1\n",
                    "evals/cases.json": b"{}\n",
                }.items()
            ):
        ...
        package_digest = _behavior_package_digest_from_zip(package_path)
        config = {
        ...
        manifest = {
            ...
            "execution_manifest_id": "scorer-self-test",
            ...
            "suites": [
                {
                    ...
                    "conditions": [
                        {
                            ...
                            "package_assurance": {
                                "mode": "artifact_verified",
                                "artifact": {
                                    "path": str(package_path.relative_to(repo)),
                    ...
                    "interpretation_scope_id": SHAKEDOWN_INTERPRETATION[
                        "interpretation_scope_id"
                    ],
                    "interpretation_rule": SHAKEDOWN_INTERPRETATION[
                        "interpretation_rule"
                    ],
                    "comparison_blocks": [
                        {
                            ...
                            "execution_config_artifact": artifact_ref(config_path) | {"schema_version": "1.1.0"},
                            "seed_control_status": "enforced",
        ...
        for case_index, case_id in enumerate(
            ...
            for order_index, condition_id in enumerate(scheduled_order, start=1):
                ...
                record = {
                    ...
                    "skill_package_sha256": package_digest if condition_id == "target" else None,
                    "case_id": case_id,
                    "replicate": 1,
                    "attempt": 1,
                    "agent": "self-test",
        ...
        passed = (
            ...
            and result.get("execution_artifact_assurance", {}).get("level")
            == "artifact_verified"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path(__file__).with_name("manifest-v2.1.1.json"),
    )
    parser.add_argument(
        "--artifact-root",
        type=Path,
