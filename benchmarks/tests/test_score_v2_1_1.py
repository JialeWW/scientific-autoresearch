...
MODULE_PATH = Path(__file__).resolve().parents[1] / "score-v2.1.1.py"
...
class BenchmarkFixture:
    def __init__(
        ...
        *,
        ...
        self.condition_specs = [
            self.package_condition("target", "test-target"),
            self.package_condition("previous", "test-previous"),
            self.package_condition("legacy", "test-legacy"),
            {
    ...
    def package_condition(self, condition_id: str, release: str) -> dict[str, object]:
        path = self.artifacts / f"{condition_id}-package.zip"
        members = {
            "SKILL.md": f"---\nname: fixture-{condition_id}\n---\n",
            "references/reference.md": f"reference {condition_id}\n",
            "scripts/validator.py": f"VALUE = {condition_id!r}\n",
            "evals/cases.json": json.dumps({"condition": condition_id}),
        }
        ...
        package_digest = score._behavior_package_digest_from_zip(path)
        return {
            ...
            "package_assurance": {
                ...
                "artifact": {
                    ...
                    "format": score.PACKAGE_ARTIFACT_FORMAT,
                },
                "attestation_artifact": None,
            },
        }
    ...
    def write_protocol(self) -> None:
        ...
        self.harness_path = self.artifacts / "fixture-harness.py"
        self.harness_path.write_text("HARNESS_VERSION = '1'\n", encoding="utf-8")
        self.config = {
            ...
            "shared_execution_config": {
                "harness_identity": {
                    ...
                    "assurance": {
                        "mode": "artifact_verified",
                        "artifact": {
                            "path": str(self.harness_path.relative_to(self.repo)),
                            "sha256": self.prefixed_sha(self.harness_path),
                ...
                blocks.append(
                    {
                        ...
                        "execution_config_artifact": self.artifact_ref(
                            self.config_path, "1.1.0"
                        ),
                        "seed_control_status": self.seed_status,
        ...
        self.manifest = {
            "benchmark_schema_version": "2.1.1",
            "benchmark_protocol_version": "2.1.1",
            "manifest_kind": "execution",
            "execution_manifest_id": "fixture-development-execution",
    ...
    def strip_outcome_fields(self, record: dict[str, object]) -> None:
        judgment_path = record.get("judgment_path")
        if isinstance(judgment_path, str):
            bound_judgment = self.evidence / judgment_path
            if bound_judgment.is_file():
                bound_judgment.unlink()
        for field in (
            "predicted_trigger",
            "routing_protocol_id",
    ...
    def remove_record_artifacts(self, record: dict[str, object]) -> None:
        for field in ("evidence_path", "judgment_path"):
...
class ScoreTests(unittest.TestCase):
    ...
    def test_complete_four_condition_blocks_produce_paired_deltas(self) -> None:
        ...
        self.assertEqual(
            result["execution_artifact_assurance"]["level"], "artifact_verified"
        )
    ...
    def test_repository_manifest_loads_without_frozen_execution_blocks(self) -> None:
        protocol = score._load_protocol(MODULE_PATH.with_name("manifest-v2.1.1.json"))
    ...
    def test_missing_condition_cell_suppresses_only_that_block_deltas(self) -> None:
        ...
        self.fixture.remove_record_artifacts(removed)
    ...
    def test_package_artifact_content_is_recomputed(self) -> None:
        target = self.fixture.condition_specs[0]
        artifact = target["package_assurance"]["artifact"]
        package_path = self.fixture.repo / artifact["path"]
        with zipfile.ZipFile(package_path, "a", compression=zipfile.ZIP_STORED) as archive:
            archive.writestr("references/added.md", b"changed package content\n")
        artifact["sha256"] = self.fixture.prefixed_sha(package_path)
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        with self.assertRaisesRegex(ValueError, "package content digest mismatch"):
            score._load_protocol(self.fixture.manifest_path)

    def test_package_artifact_rejects_absolute_and_parent_paths(self) -> None:
        target = self.fixture.condition_specs[0]
        artifact = target["package_assurance"]["artifact"]
        package_path = self.fixture.repo / artifact["path"]
        artifact["path"] = str(package_path.resolve())
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
    ...
    def test_package_artifact_rejects_symlinked_ancestor(self) -> None:
        alias = self.fixture.repo / "artifact-alias"
        alias.symlink_to(self.fixture.artifacts, target_is_directory=True)
        target = self.fixture.condition_specs[0]
        artifact = target["package_assurance"]["artifact"]
        package_path = self.fixture.repo / artifact["path"]
        artifact["path"] = f"artifact-alias/{package_path.name}"
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        with self.assertRaisesRegex(ValueError, "cannot traverse a symbolic link"):
            score._load_protocol(self.fixture.manifest_path)

    def test_package_zip_rejects_symlink_and_special_members(self) -> None:
        for file_type, message in (
            (stat.S_IFLNK, "symbolic links"),
            (stat.S_IFIFO, "special files"),
        ):
            package_path = self.fixture.artifacts / f"unsafe-{file_type}.zip"
            with zipfile.ZipFile(
                package_path, "w", compression=zipfile.ZIP_STORED
            ) as archive:
                info = zipfile.ZipInfo("references/unsafe.md")
                info.create_system = 3
                info.external_attr = (file_type | 0o644) << 16
                archive.writestr(info, b"unsafe")
            with self.assertRaisesRegex(ValueError, message):
                score._behavior_package_digest_from_zip(package_path)

        package_path = self.fixture.artifacts / "unsafe-symlink-directory.zip"
        with zipfile.ZipFile(
            package_path, "w", compression=zipfile.ZIP_STORED
        ) as archive:
            info = zipfile.ZipInfo("references/")
            info.create_system = 3
        ...
        package_path = self.fixture.artifacts / "explicit-directory.zip"
        with zipfile.ZipFile(
            package_path, "w", compression=zipfile.ZIP_STORED
        ) as archive:
            info = zipfile.ZipInfo("references/")
            ...
            archive.writestr(info, b"")
        with self.assertRaisesRegex(ValueError, "explicit directory members"):
            score._behavior_package_digest_from_zip(package_path)

    def test_package_zip_rejects_canonical_path_collisions(self) -> None:
        package_path = self.fixture.artifacts / "colliding-paths.zip"
        with zipfile.ZipFile(
            package_path, "w", compression=zipfile.ZIP_STORED
        ) as archive:
            archive.writestr("references/r.md", b"one")
            archive.writestr("references/./r.md", b"two")
        with self.assertRaisesRegex(ValueError, "noncanonical member path"):
            score._behavior_package_digest_from_zip(package_path)

        package_path = self.fixture.artifacts / "case-colliding-paths.zip"
        with zipfile.ZipFile(
            package_path, "w", compression=zipfile.ZIP_STORED
        ) as archive:
            archive.writestr("SKILL.md", b"fixture")
            archive.writestr("references/A.md", b"one")
            archive.writestr("references/a.md", b"two")
            archive.writestr("scripts/check.py", b"VALUE = 1\n")
            archive.writestr("evals/cases.json", b"{}")
        with self.assertRaisesRegex(ValueError, "colliding member paths"):
            score._behavior_package_digest_from_zip(package_path)

    def test_artifact_root_rejects_symbolic_link(self) -> None:
        alias = self.fixture.root / "repository-alias"
        alias.symlink_to(self.fixture.repo, target_is_directory=True)
        with self.assertRaisesRegex(ValueError, "artifact root cannot be"):
            score._load_protocol(self.fixture.manifest_path, artifact_root=alias)

    def test_manifest_rejects_symbolic_link(self) -> None:
        alias = self.fixture.benchmarks / "manifest-link.json"
        alias.symlink_to(self.fixture.manifest_path)
        with self.assertRaisesRegex(ValueError, "manifest cannot be a symbolic link"):
            score._load_protocol(alias)

    def test_harness_artifact_bytes_are_recomputed(self) -> None:
        self.fixture.harness_path.write_text("HARNESS_VERSION = 'changed'\n", encoding="utf-8")
        self.fixture.config_path.write_text(
            json.dumps(self.fixture.config), encoding="utf-8"
        )
        for suite in self.fixture.manifest["suites"]:
            suite["comparison_blocks"][0]["execution_config_artifact"][
                "sha256"
            ] = self.fixture.prefixed_sha(self.fixture.config_path)
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        with self.assertRaisesRegex(ValueError, "harness assurance artifact SHA-256 mismatch"):
            score._load_protocol(self.fixture.manifest_path)

    def test_attested_package_lowers_reported_assurance(self) -> None:
        target_package_digest = self.fixture.condition_specs[0][
            "skill_package_sha256"
        ]
        attestation = {
            "assurance_attestation_schema_version": "1.0.0",
            "attestation_id": "target-package-attestation",
            "subject_kind": "skill_package",
            "subject_id": "target",
            "immutable_reference": target_package_digest,
            "statement": "The runner attests that this package identity was supplied.",
            "issued_by": "fixture-runner",
            "issued_at": "2000-01-01T00:00:00Z",
        }
        path = self.fixture.artifacts / "target-package-attestation.json"
        path.write_text(json.dumps(attestation), encoding="utf-8")
        self.fixture.condition_specs[0]["package_assurance"] = {
            "mode": "attested",
            "artifact": None,
            "attestation_artifact": self.fixture.artifact_ref(path),
        }
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        ...
        result = self.fixture.score(self.fixture.all_records())
        self.assertTrue(result["valid"])
        self.assertEqual(result["execution_artifact_assurance"]["level"], "attested")
        self.assertEqual(
            result["block_results"][0]["execution_artifact_assurance"]["level"],
            "attested",
        )
        self.assertEqual(
            result["execution_artifact_assurance"]["runtime_use_assurance"],
            "unreported",
        )

    def test_attestation_must_bind_material_and_precede_freeze(self) -> None:
        target = self.fixture.condition_specs[0]
        attestation = {
            "assurance_attestation_schema_version": "1.0.0",
            "attestation_id": "target-package-attestation",
        ...
        with self.assertRaisesRegex(ValueError, "immutable_reference mismatch"):
            score._load_protocol(self.fixture.manifest_path)

        attestation["immutable_reference"] = target["skill_package_sha256"]
        attestation["issued_at"] = "2099-01-01T00:00:00Z"
        path.write_text(json.dumps(attestation), encoding="utf-8")
        target["package_assurance"]["attestation_artifact"] = (
            self.fixture.artifact_ref(path)
        )
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
    ...
    def test_shakedown_cannot_emit_behavioral_evidence_or_metrics(self) -> None:
        self.fixture.manifest["execution_manifest_id"] = "fixture-shakedown"
        self.fixture.manifest["execution_purpose"] = "execution_shakedown"
        for suite in self.fixture.manifest["suites"]:
            suite["case_source"]["role"] = "shakedown"
            ...
            for block in suite["comparison_blocks"]:
                block["evidence_status_on_complete"] = "not_evaluated"
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        self.fixture.protocol = score._load_protocol(self.fixture.manifest_path)
        result = self.fixture.score(self.fixture.all_records())
        self.assertTrue(result["valid"])
        self.assertEqual(result["execution_purpose"], "execution_shakedown")
        ...
        self.assertTrue(
            all(not block["paired_deltas_reported"] for block in result["block_results"])
        )

    def test_evidence_tree_rejects_orphans_and_symbolic_links(self) -> None:
        records = self.fixture.all_records()
        orphan = self.fixture.evidence / "orphan.txt"
        orphan.write_text("unrecorded output\n", encoding="utf-8")
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalid")
        self.assertTrue(any("unreferenced files" in item for item in result["errors"]))
        orphan.unlink()

        link = self.fixture.evidence / "linked-output.txt"
        link.symlink_to(self.fixture.evidence / str(records[0]["evidence_path"]))
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalid")
        ...
        if hasattr(os, "mkfifo"):
            fifo = self.fixture.evidence / "special-output.fifo"
            os.mkfifo(fifo)
            result = self.fixture.score(records)
            self.assertEqual(result["scoring_status"], "invalid")
            self.assertTrue(
                any("regular" in item or "special" in item for item in result["errors"])
            )
            fifo.unlink()

    def test_evidence_and_judgment_cannot_share_one_file(self) -> None:
        records = self.fixture.all_records()
        source = records[0]
        other = records[1]
        original_evidence = self.fixture.evidence / str(source["evidence_path"])
        original_evidence.unlink()
        source["evidence_path"] = other["judgment_path"]
        source["evidence_sha256"] = other["judgment_sha256"]
        self.fixture.recompute_identity(source)
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalid")
        self.assertTrue(
            any("referenced more than once" in item for item in result["errors"])
        )

    def test_record_paths_must_be_json_strings(self) -> None:
        records = self.fixture.all_records()
        records[0]["evidence_path"] = 1
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalid")
        self.assertTrue(any("must be a string path" in item for item in result["errors"]))

    def test_json_parsers_reject_duplicate_object_keys(self) -> None:
        with self.assertRaisesRegex(ValueError, "duplicate JSON object key"):
            score._json_from_bytes(b'{"a": 1, "a": 2}', "fixture JSON")
        with self.assertRaisesRegex(ValueError, "duplicate JSON object key"):
            score._jsonl_from_bytes(b'{"a": 1, "a": 2}\n', "fixture JSONL")

    def test_execution_and_scoring_times_follow_the_freeze(self) -> None:
        records = self.fixture.all_records()
        self.fixture.manifest["frozen_at"] = "2099-01-01T00:00:00Z"
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalidated")
        self.assertTrue(any("precedes the execution freeze" in item for item in result["errors"]))

        self.fixture.manifest["frozen_at"] = "2000-01-01T00:00:00Z"
        record = records[0]
        record["scored_at"] = record["started_at"]
        ...
        result = self.fixture.score(records)
        self.assertEqual(result["scoring_status"], "invalidated")
        self.assertTrue(any("scored_at precedes" in item for item in result["errors"]))

    def test_behavioral_cases_require_a_critical_assertion(self) -> None:
        behavior_path = self.fixture.cases / "behavior.json"
        for case in self.fixture.behavior_cases["evals"]:
            case["critical_assertion_ids"] = []
        behavior_path.write_text(
            json.dumps(self.fixture.behavior_cases), encoding="utf-8"
        )
        behavior_suite = next(
            suite
            for suite in self.fixture.manifest["suites"]
            if suite["suite_id"] == "behavior"
        )
        behavior_suite["case_source"]["sha256"] = self.fixture.prefixed_sha(
            behavior_path
        )
        self.fixture.manifest_path.write_text(
            json.dumps(self.fixture.manifest), encoding="utf-8"
        )
        with self.assertRaisesRegex(ValueError, "invalid critical IDs"):
            score._load_protocol(self.fixture.manifest_path)
    ...
    def test_actual_start_order_mismatch_blocks_comparison(self) -> None:
        ...
        target["scored_at"] = target["completed_at"]
        previous["scored_at"] = previous["completed_at"]
