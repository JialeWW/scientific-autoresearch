...
Start after the proportionate support, data-semantic, execution-risk, and, when applicable, governance checks needed to interpret the work. Freeze the full required protocol for confirmatory, blinded, costly, irreversible, regulated, safety-sensitive, or jointly inferred work. For other work, specify the next informative test or coherent batch unless a complete data-to-decision mapping can already be justified and fixed; defer unsupported later stages.

Continue across multiple result–decision rounds only when the user explicitly authorizes autonomous or iterative investigation. For design, read-only audit, or one named test, complete the requested scope and list further scientific tests as proposals. Within an authorized iterative scope, define a round as a scientific checkpoint; tool calls, retries, jobs, and routine confirmation events remain operational events. Seek new authorization when scope, cost, risk, data access, or external action would change.
...
Keep each entry compact and record at least its rationale, distinct prediction, required support, observable and scale, expected signature, falsifier, status, and next action. Keep plausible but unsupported candidates as `needs_data`. Treat proxies, thresholds, parameter values, and Cartesian-product combinations as distinct scientific candidates only when they have distinct scientific roles.

## 2. Freeze and run the next informative test

Freeze the next executable test or coherent prespecified batch. If a complete multi-stage data-to-decision mapping can already be fixed, freeze it once; introduce successors only when that mapping changes. Specify formulations, data and exclusions, support, observable, method, scale rule, comparator, uncertainty, falsifier, decision role, stopping, and reporting. A finite family or automated procedure may be one batch when its complete mapping is fixed before outcomes.
...
Choose the unresolved candidate, falsifier, validation, or refinement most likely to change scientific understanding. Run the simplest valid supported test that distinguishes plausible alternatives with different observable predictions. Base selection on expected scientific informativeness; nominal significance and convenience alone are insufficient criteria.
...
When a protocol or builder declares stable scientific identifiers or science-facing data-product families, keep their dispositions in the compact record. Reconcile each declared candidate, prediction, falsifier, control, scientific role, and science-facing product to a test and output, an explicit closure, or the open queue. Deterministic reconciliation applies only to declared identifiers and resolvable references; free-text semantic equivalence requires scientific review. Record each product family's role and science-facing status so QA, intermediate, and provenance fields are not treated as scientific blockers. Labels such as `sensitivity` and `diagnostic` define scientific roles and do not constitute dispositions.

If a viewed outcome motivates an unplanned candidate, formulation, sample, method, threshold, ranking, stopping, or reporting change, preserve the earlier result, record what was seen and why it changed the next step, and freeze the successor before running it. Classify the change as outcome-informed in all subsequent reporting.
...
## 5. Requirements for conclusion promotion

Conduct initial tests promptly. Before promoting a conclusion:
...
Treat "Does any qualifying next test remain?" or "Is anything actionable left?" as continuation questions. Answer from `search_stop_admissible` and name any current qualifying open test; when explicit coverage is active, also give the exact open queue. A true state means no currently identified, supported, feasible, authorized, potentially material test meets the continuation rule; false means name the qualifying open test; `indeterminate` means name the unresolved dispute; and `not_assessed` means state that scientific stopping was not assessed.
...
Distinguish scoped-completion questions from continuation questions. Treat "Is the search exhaustive?" or "Have we exhausted the declared possibilities?" as scoped-completion questions. Answer from `complete_within_scope`, not from `inventory_saturated`, `coverage_complete`, `request_execution_complete`, or `search_stop_admissible` alone. If `complete_within_scope=false`, state that scoped completion has not been established and name the failed gate even when the current inventory is saturated and every declared cell is closed; never say "yes within scope" in that state. If it is `not_assessed`, state that scoped completeness was not assessed. If it is true, state that the declared versioned scope is complete and explicitly specify that this status does not establish open-world exhaustiveness. If wording such as "Is anything left?" is ambiguous, report both `search_stop_admissible` and `complete_within_scope` rather than inferring either from the other.
...
Prohibited conduct includes omitting, relabeling, redefining, or selectively reporting data, attempts, candidates, scales, thresholds, subgroups, transformations, models, or results to conceal evidence that weakens a claim; ranking by nominal p-value alone; representing outcome-driven adaptation as confirmation or same-data variants as independent verification; fabricating data, citations, approvals, provenance, commands, results, evidence, or coverage; exceeding authorization; and exposing restricted information. Document legitimate corrections and definition changes.
