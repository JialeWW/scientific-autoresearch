...
This is an audit and validator-diagnosis reference, not default Agent context. Its profile, stage, record-mode, and completion names are compatibility vocabulary for structured schema-1.5.4 records; ordinary scientific work follows the core semantic boundaries without emitting these labels. For schema `1.5.4`, the initializer and `scripts/validate_run.py` are authoritative for fields, enums, and transition checks. Do not copy this page into every run.
...
Profiles are ordered `fixed_test < adaptive_search < coverage_search`. A frozen family or automated procedure remains `fixed_test` when outcome-dependent transitions stay inside its prespecified data-to-decision mapping. Upgrade only when inspected outcomes change membership, methods, thresholds, sample, stopping, multiplicity or joint rule, decision role, or reporting outside that frozen procedure. `profile_history` is append-only and its offset-aware `started_at` values are strictly increasing. Before an upgrade, validate the source and run `scripts/validate_run.py --snapshot-upgrade RUN_DIR --to-profile PROFILE`. The returned upgrade entry records `preservation_snapshot_path` and `preservation_snapshot_sha256`; append it only after reviewing the non-overwriting copy. A preservation snapshot declares the prior profile, capture time, target profile, and an artifact list whose entries contain `logical_path`, internal nonsymlink `snapshot_path`, and verified `sha256`. It must include the prior manifest, that profile's required artifacts, and every round report and reproduction record indexed by the prior manifest. Boolean preservation attestations alone are insufficient. A downgrade is invalid. A changed sample, codebase, model, tool, workflow, or skill version never restores confirmatory status.

Schema 1.5.2 `skill_provenance` is an ordered history automatically seeded by initialization. Before authorized execution or resume, use `scripts/validate_run.py --record-skill-provenance RUN_DIR`; unchanged identity is a no-op and changed identity appends a new entry. Read-only validation never edits it. Each entry records the skill name, release version, deterministic runtime-package SHA-256, offset-aware capture time, and an optional source revision. For v0.3.0 the digest binds `SKILL.md`, bundled references, and scripts while excluding run outputs and repository-only historical evals; symbolic links on the runtime surface are invalid. It identifies content but does not prove compliance or independent timing. Older releases retain the digest semantics of the validator that recorded them.
...
Mutable development and response-blind execution qualification remain outside production `profile_history`; only outcome-adaptivity changes belong there.
...
- For a machine-audited run, use the validator before a material decision-bearing resume and before any stage, pause, or completion report. Ordinary retries, engineering resumes, and accepted execution amendments do not require repeated full validation. Metadata consistency and numerical reproduction are separate claims.
